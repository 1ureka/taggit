...
- [x] 預期：成功切換；側欄顯示 committed 0 / staged 0；首頁為空清單。
- [x] 錯誤路徑：填一個不存在且無法建立的路徑 → 422「路徑不存在或無法建立所需的子目錄」。

## 2. 放入圖片（檔案系統）

- [x] 手動丟 5–10 張圖到 `<collection>/images/`。
- [x] 回到 app（任一頁重新整理）→ 側欄 staged 數量 = 丟入張數。

## 3. tagger 匯入

- [x] 開 `/tagger`，逐張（或匯入 JSON）給名稱、標籤（至少兩種標籤、部分共用）、評分後提交。
- [x] 預期：每張成功後從 staged 清單消失；側欄 committed/staged 數字同步變化。
...
- [x] 點圖開 modal → 點「在 editor 開啟」→ editor 帶著同樣篩選 + currentId 開啟。

## 5. editor 編修

- [x] 單張：改名稱 / 加減標籤 / 改評分 → Ctrl+S 存檔 → toast「已存檔」，清單即時更新。
- [ ] 批次：多選（Ctrl / Shift）→ 批次加減標籤 → 確認後套用。
- [x] 篩選對話框：套條件、再按重置 → 條件全清但 `currentId` 保留。
- [ ] 錯誤路徑：
...
- [x] 改名：把常用標籤 A 改名 B → 所有圖的 A 變 B；再把 B 改回 A。
  - [x] 改名為圖片已有的另一標籤（合併場景）→ 成功、無重複標籤。
...
- [x] 刪除：
  - [x] 刪一個「所有持有圖片都還有其他標籤」的標籤 → 成功，`affected` = 圖片數。
  - [x] **last_tag**：刪某張圖的唯一標籤 → 409 `last_tag`，`images` 列出受影響 id；
        前端顯示「有圖片會因此失去最後一個標籤」。
- [x] hidden（settings 頁）：
  - [x] 把標籤 H 設為隱藏 → home 空查詢看不到帶 H 的圖；facet 中 H 的計數 =「解鎖後會有幾張」。
  - [x] 查詢明確 include H → 圖片現身。
  - [x] settings 的 authoring 清單（不遮蔽、含未使用標籤）仍列出 H。
  - [x] 取消隱藏 → 圖片全部回來。

## 7. compare

- [x] 開 `/compare` → 每次重新整理隨機兩張。
- [x] 帶篩選參數進 compare（如 `?includedTags=A`）→ 兩張都符合條件。
- [x] hidden 標籤的圖不會出現在隨機對中（除非 include 解鎖）。

## 8. player

- [x] 從有結果的篩選開 `/player` → 正常輪播。
- [x] 手改 URL 成零結果的條件 → 302 導回 `/`（帶原查詢參數）。
- [x] hidden 標籤的圖不會出現（除非 include 解鎖）。

## 9. settings 維護掃描

- [x] metadata：`GET /api/settings/metadata` 檢查缺元資料數量 → 補算 → 數量歸 0。
...
- [x] 把 collection path 指到無效位置（或重啟 dev server 後不進任何頁面直接打 API）：頁面路由則導向 `/settings?alert=...`。

## 11. 持久化收尾

- [x] 做完任一寫入後等 ≥ 500ms，確認 `db.json` 內容更新（防抖寫入）。
- [x] `Ctrl+C` 關掉 dev server → log 出現「接收到 SIGINT 訊號，正在寫入資料庫…」且 `db.json` 為最新。
