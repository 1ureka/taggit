...
/**
 * 標籤查詢的回傳實體，對標 {@link ImageWithId}：
 * `name` 為身份、`count` 為查詢衍生的命中數、`meta` 為標籤自身設定（已補齊預設）。
 */
export interface Tag {
  /** 標籤名稱（唯一識別） */
  name: string;
  /** 命中數；語義依 {@link TagQueryOptions.hidden} 而定 */
  count: number;
  /** 標籤自身的元資料（補齊預設值） */
  meta: TagMeta;
}
...
/**
 * 標籤查詢的組合選項。
 * 非 URL 參數，而是由呼叫端依語境（篩選 vs 編寫／管理）自行決定。
 */
export interface TagQueryOptions {
  /**
   * hidden 標籤的處理方式：
   * - `"mask"`（預設，篩選語境）：套用 hidden 遮蔽；對 hidden 且不在查詢 includedTags 內的標籤，
   *   `count` 以「將其加入篩選後的可見圖片數」計，供篩選 UI 顯示點擊後的結果數。
   * - `"ignore"`（編寫／管理語境）：不遮蔽，`count` 為 scope 內（未遮蔽）擁有該標籤的圖片數。
   */
  hidden?: "mask" | "ignore";
  /**
   * 納入的標籤範圍：
   * - `"used"`（預設）：只列出目前 scope 內被使用的標籤。
   * - `"all"`：額外併入僅有元資料、未被任何圖片使用的標籤（`count` 為 0）。
   */
  universe?: "used" | "all";
}
