 ...
 *
 * faceted 頁 = 同一個 ImageWhere 各建 ImageQuery 與 TagQuery，對同一個 Query 實例
 * 分別呼叫 images / tags（兩引擎不互傳結果）。
 */

import type { Database, ImageWithId } from "../database/index.js";
import type { ImageQuery, TagQuery } from "../query-spec/index.js";
...
export class Query {
  ...
  /** 標籤查詢引擎：scope present/absent 決定 count 與遮蔽 → 排序 → 分頁。 */
  tags(q: TagQuery): QueryResult<Tag> {
    return this.tagEngine.run(q);
  }
