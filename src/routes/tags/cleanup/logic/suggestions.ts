 ...
 * （名稱近似、高度共現、冷門標籤、零使用標籤）。純函式，不碰 Database，
 * 前後端皆可 import；由 `+page.server.ts` 在 load 時呼叫，隨每次讀取重新計算，不快取。
 */
...
/**
 * 建立標籤清理建議清單。
 * `images` 需為完整、未經 hidden 遮蔽的原始圖片清單（維護性掃描用途，見 {@link Query.getAllImages}），
 * `tags` 需為 `universe: "all"` 的完整標籤清單（含 count 0 的純元資料標籤）。
 */
export function buildTagCleanupSuggestions(tags: Tag[], images: ImageWithId[]): Suggestion[] {
  ...
  for (const bucket of buckets.values()) {
    for (let i = 0; i < bucket.length; i++) {
      for (let j = i + 1; j < bucket.length; j++) {
        ...
        if (!withinEditDistance1(normalize(bucket[i]), normalize(bucket[j]))) continue;
        pairSeen.add(key);
        suggestions.push({
          id: `similar:${key}`,
          kind: "similar",
