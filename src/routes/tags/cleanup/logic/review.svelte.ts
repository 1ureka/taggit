...
class ReviewController {
  ...
  /** 全部有排入操作的標籤，維持插入順序，**不應加入任何過濾** */
  private names = $derived(this.schedule.operations.map((op) => op.name));
  /** 一輪能承擔的審查量 */
  ...
  /**
   * 指定標籤不可送出的原因，可送出時為 `null`。
   *
   * 結構問題優先於上次送出失敗。合併在這裡而不是留給元件各自判斷，
   * 是為了讓畫面顯示的可勾選狀態與 `submittableNames` 永遠出自同一個判斷。
   */
  ...
  checkableOf(name: string): boolean {
    return this.problemOf(name) === null;
  }
  /** 指定標籤目前是否已勾選 */
  ...
  /** 送出本批可送出的項目；成功的會被 submit 自己自排程移除，這裡只負責同步 checked 狀態 */
  handleSubmit = async () => {
