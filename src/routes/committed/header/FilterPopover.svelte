...
<Popover {open} {reference} placement="bottom-start">
  <div bind:this={panelRef} class="panel">
    <TagInput
      tags={query.query.where.includedTags}
      scope={query.facetScope}
      label="包含的標籤"
      onchange={(tags) => query.handleTagsChange("includedTags", tags)}
    />
    <TagInput
      tags={query.query.where.excludedTags}
      scope={query.facetScope}
      label="排除的標籤"
      onchange={(tags) => query.handleTagsChange("excludedTags", tags)}
    />
    <div>
      <span>評等</span>
      <div>
        <Select
          ...
          value={query.query.where.ratingOp}
          onchange={query.handleRatingOpChange}
        />
        <Select
          id="{id}-rating"
          aria-label="評等"
          ...
          option={ratingOption}
          value={query.query.where.rating ? String(query.query.where.rating) : "all"}
          onchange={query.handleRatingChange}
        />
      </div>
    </div>
  </div>
