/**
 * `POST /api/committed/import`
 *
 * 匯入圖片紀錄至資料庫。接收 JSON body，逐筆驗證並處理，
 * 以 SSE (Server-Sent Events) 串流回傳即時進度。
 *
 * Body 格式: `Record<filename, { name: string; tags: string[]; rating?: number }>`
 *
 * SSE 事件格式:
...
function validateEntry(
  ...
  return {
    ok: true,
    entry: { name: name as string, tags: tags as string[], rating: resolvedRating as number },
  };
}
